...
.DESCRIPTION
  Fast checks with light heuristics to avoid stale WSC false-negatives.
  Default output is a one-liner; use -Full for ticket notes; -AsJson for parsing.
  Covers: AV (Defender/Bitdefender/3rd‑party), Firewall, MDE, Windows Update currency,
          Pending reboot, Last reboot/uptime.
  Exit codes: 0=Secure, 1=Warning, 2=Critical
#>

[CmdletBinding()]
param(
  # ===== Output modes =====
  [switch]$Full, [switch]$AsJson,
  ...
  # ===== Thresholds =====
  [int]$StaleHours = 72, [int]$SigFreshHours = 48,
  [int]$MaxQuickScanAgeDays = 14, [int]$MaxFullScanAgeDays = 30,
  [int]$WUCurrencyDays = 30,

  # ===== Minimum requirements =====
  [switch]$RequireFirewallOn = $true, [switch]$RequireRealTime = $false, [switch]$RequireMDE,
  ...
  [string]$ForceVendor = '', [switch]$ForceVendorPresent,
...
# Map common vendor -> service name patterns to improve detection (used in Get-VendorServiceStatus)
...
function Get-VendorServiceStatus { # Report 3rd‑party service state for summary (e.g. AVGSvc: Running)
...
# ----------------------------- System health -----------------------------
$fwState='Unknown'
try{ $fw=Get-Service mpssvc -ErrorAction Stop; $fwState= if($fw.Status -eq 'Running'){'On'}else{'Off'} }
...
try{
  $qfe=Get-WmiObject -Class Win32_QuickFixEngineering -ErrorAction Stop | Sort-Object InstalledOn -Descending | Select-Object -First 1
...
try{ $boot=(Get-CimInstance -ClassName Win32_OperatingSystem).LastBootUpTime; $upt=((Get-Date)-[datetime]$boot).TotalDays; $lastBootString=("{0} (uptime {1:N0} days)" -f $boot,$upt); if($upt -gt 45){ Add-Issue ("System uptime exceeds {0:N0} days" -f $upt) 'Warning' } }catch{ $lastBootString='Unknown' }
...
# Third‑party service status in summary (e.g., AVGSvc: Running)
$avSvc='Unknown'
...
elseif ($label -eq 'Other 3rd-party AV' -and $activeThird) {
  ...
  if ($svcStatus -ne 'Unknown') {
    ...
    $svcName = if ($svcObj) { $svcObj.Name } else { $vendorName }
    $avSvc   = "{0}: {1}" -f $svcName, $svcStatus  # e.g., AVGSvc: Running
  }
}
...
$result=[pscustomobject]@{
  ...
  Summary=[pscustomobject]@{ ActiveAV=$summaryAV; AVService=$avSvc; MDE=$mdeLine; Firewall=$fwState; WindowsUpdateLast=$WU_Last; PendingReboot=$RebootPending; LastReboot=$lastBootString }
  ...
  Parameters=[pscustomobject]@{ StaleHours=$StaleHours; SigFreshHours=$SigFreshHours; MaxQuickScanAgeDays=$MaxQuickScanAgeDays; MaxFullScanAgeDays=$MaxFullScanAgeDays; WUCurrencyDays=$WUCurrencyDays
    RequireFirewallOn=[bool]$RequireFirewallOn; RequireRealTime=[bool]$RequireRealTime; RequireMDE=[bool]$RequireMDE; Strict=[bool]$Strict
...
if($AsJson){
  ...
  Write-Output ("[{0}] {1} | Status:{2} | AV:{3} | Svc:{4} | MDE:{5} | FW:{6} | WU:{7} | PendingReboot:{8} | LastReboot:{9}{10}" -f `
    $result.Timestamp,$result.ComputerName,$result.Status,$result.Summary.ActiveAV,$result.Summary.AVService,$result.Summary.MDE,$result.Summary.Firewall,$result.Summary.WindowsUpdateLast,$result.Summary.PendingReboot,$result.Summary.LastReboot,$issuesText)
}else{
  ...
  Write-Host ("MDE: {0}" -f $result.Summary.MDE)
  ...
  Write-Host "`n--- Microsoft Defender ---"
  ...
  if($products.Count -eq 0){ Write-Host "  (none reported)" } else {
    $products | ForEach-Object {
      ...
      if ($_.ReportingExe) { Write-Host ("    ReportingExe: {0}" -f $_.ReportingExe) }
      if ($_.Timestamp)    { Write-Host ("    Timestamp:    {0}" -f $_.Timestamp) }
    }
  }

  Write-Host "`n--- MDE (Sense) ---"
  Write-Host ("  Present:{0}  Status:{1}  Onboarded:{2}" -f (BoolStr $mde.SensePresent), $mde.SenseStatus, (BoolStr $mde.Onboarded))

  if($bitdef){
    Write-Host "`n--- Bitdefender BEST ---"
    Write-Host ("  Version:{0}" -f $bitdef.Version)
    Write-Host ("  InstallPath:{0}" -f $bitdef.InstallPath)
    Write-Host ("  ServiceExe:{0}" -f $bitdef.ServiceExe)
  ...
  Write-Host ("  RequireFirewallOn:{0} RequireRealTime:{1} RequireMDE:{2} Strict:{3}" -f ([bool]$RequireFirewallOn), ([bool]$RequireRealTime), ([bool]$RequireMDE), ([bool]$Strict))
  Write-Host ("  ForceClassification:{0} ForceFirewall:{1} ForceSigUpToDate:{2} ForceSense:{3} AssumeDefenderRTWhenServiceRunning:{4}" -f $ForceClassification, $ForceFirewall, $ForceSigUpToDate, $ForceSense, ([bool]$AssumeDefenderRTWhenServiceRunning))
  Write-Host ("  ForceVendor:{0} ForceVendorPresent:{1} ForceVendorServiceStatus:{2}" -f $ForceVendor, ([bool]$ForceVendorPresent), $ForceVendorServiceStatus)
}
