...
function Get-ExeBitness {
    # Read the PE header Machine field to determine true architecture.
    # This is the same data source N-able's agent uses, so it will always
    # agree with what N-able reports in the Architecture column.
    # Machine values: 0x014C = x86 (32-bit), 0x8664 = x64 (64-bit), 0xAA64 = ARM64 (64-bit)
    param([string]$Path)
    try {
        $bytes    = [System.IO.File]::ReadAllBytes($Path)
        $peOffset = [BitConverter]::ToInt32($bytes, 0x3C)
        $machine  = [BitConverter]::ToUInt16($bytes, $peOffset + 4)
        if ($machine -eq 0x8664 -or $machine -eq 0xAA64) { return "64-bit" }
        if ($machine -eq 0x014C)                          { return "32-bit" }
        return "Unknown"
    } catch {
        return "Unknown"
    }
...
foreach ($path in $firefoxExePaths) {
    ...
    if ($ver) {
        $firefoxFound = $true
        $arch         = Get-ExeBitness $path
        $installType  = if ($path -like "*Users*") { "Per-User" } else { "System" }
...
foreach ($r in $Results) {
    ...
    if ($r.Location -ne "Not Found") {
        ...
        Write-Output "  Version      : $($r.Version)  ($($r.Architecture))"
...
# ── N-able summary lines ──────────────────────────────────────────────────────
# Two lines per installed browser:
#   1. Machine-parseable line — matches the Python report parser exactly.
#      Format: {Full Browser Name}  {C:\path\exe}  {version}  {32-bit|64-bit}  {System|Per-User}
#   2. Rich detail line — human-readable, carries install method, policy, update health etc.
Write-Output "--- Summary ---"
foreach ($r in $Results) {
    $healthStr = if ($r.Browser -eq "Google Chrome" -and $chromeHealth) { " | UpdateHealth: $($chromeHealth.Status)" } else { "" }
    if ($r.Location -ne "Not Found") {
        # Parseable line — keep this format exactly; the Python report regex depends on it
        $scope = if ($r.InstallType -like "*Per-User*") { "Per-User" } else { "System" }
        Write-Output "$($r.Browser)  $($r.Location)  $($r.Version)  $($r.Architecture)  $scope"
        # Rich detail line
        Write-Output "$($r.Browser) INSTALLED | Type: $($r.InstallType) | Method: $($r.InstallMethod) | Publisher: $($r.Publisher) | Version: $($r.Version) | Arch: $($r.Architecture) | Policy: $($r.PolicyManaged) | UpdateDetail: $($r.UpdateDetail) | UpdateSvc: $($r.UpdateService) | UpdateTasks: $($r.UpdateTasks) | Path: $($r.Location)$healthStr"
    } else {
